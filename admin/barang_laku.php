...
<h3><span class="glyphicon glyphicon-briefcase"></span>  Data Barang Masuk/Keluar</h3>
...
<form action="" method="get">
	<div class="input-group col-md-5 col-md-offset-7">
		...
		<select type="submit" name="tanggal" class="form-control" onchange="this.form.submit()">
			...
			<?php 
			$pil=mysql_query("select distinct tanggal from barang_masuk_keluar order by tanggal desc");
...
<?php 
if(isset($_GET['tanggal'])){
	echo "<h4> Data Barang Masuk/Keluar Tanggal  <a style='color:blue'> ". $_GET['tanggal']."</a></h4>";
}
?>
<table class="table">
	<tr>
	...
	if(isset($_GET['tanggal'])){
		$tanggal=mysql_real_escape_string($_GET['tanggal']);
		$brg=mysql_query("select * from barang_masuk_keluar where tanggal like '$tanggal' order by tanggal desc");
	}else{
		$brg=mysql_query("select * from barang_masuk_keluar order by tanggal desc");
	}
	$no=1;
	while($b=mysql_fetch_array($brg)){
		...
		<tr>
			...
			<td>		
				...
				<a onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){ location.href='hapus_laku.php?id=<?php echo $b['id']; ?>&nama=<?php echo $b['nama']; ?>' }" class="btn btn-danger">Hapus</a>
			</td>
		</tr>
	...
	<tr>
		...
		if(isset($_GET['tanggal'])){
			$tanggal=mysql_real_escape_string($_GET['tanggal']);
			$x=mysql_query("select sum(masuk) as total from barang_masuk_keluar where tanggal='$tanggal'");	
	...
	<tr>
		...
		if(isset($_GET['tanggal'])){
			$tanggal=mysql_real_escape_string($_GET['tanggal']);
			$x=mysql_query("select sum(keluar) as total from barang_masuk_keluar where tanggal='$tanggal'");	
			$xx=mysql_fetch_array($x);			
			echo "<td><b>". number_format($xx['total'])."</b></td>";
		}else{
